...
namespace SocialNetworkBe.Services.PostServices
{
    public class PostService : IPostService
    {
        ...
        private readonly IUploadService _uploadService; // ✅ Thêm UploadService
        ...
        public async Task<(CreatePostEnum, Guid?)> CreatePostAsync(CreatePostRequest request, Guid userId)
        {
            try
            {
                ...
                if (result > 0)
                {
                    _logger.LogInformation("Post created successfully with {ImageCount} images for user {UserId}",
                        imageUrls?.Count ?? 0, userId);
